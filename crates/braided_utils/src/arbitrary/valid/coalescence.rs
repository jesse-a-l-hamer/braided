...
fn can_slide_to_right_of_band(band: Letter, left: Letter) -> bool {
    match band.sign() {
        Sign::Positive => {
            (left.foot() == band.foot() && left.head() < band.head())
                || (left.foot() < band.foot() && left.head() == band.head())
                || left.foot() == band.head()
        }
        Sign::Negative => {
...
fn get_pruning_safety(
    ...
    if (check_first && pair_extends_band(first, first_left_letter.unwrap(), *second_left_letter))
        ...
        || can_slide_to_right_of_band(second, *first_right_letter)
    {
        PruningSafety::None
...
fn of_band_letter_walled(
    ...
    of_band_letter(max_head, max_height, Some(max_band_artin_length))
        ...
        .prop_filter(
            "The left wall letter may not slide past the band.",
            |(band, _, _, _, left_wall_letter, _)| {
                !can_slide_to_right_of_band(*band, *left_wall_letter)
            },
        )
        .prop_filter(
...
pub fn of_band_generator_with_canonical_decomposition(
    max_head: Option<u16>,
    max_height: Option<u16>,
    max_artin_length: Option<u16>,
) -> impl Strategy<Value = (BandResult, Vec<ArtinGenerator>)> {
    ...
    valid::band::data(max_head, max_height, max_artin_length).prop_map(|(foot, head, sign)| {
        let band_generator = BandGenerator::try_new(foot, head, sign);
        let band_foot: u16 = foot.try_into().unwrap();
        let crossing_foot: u16 = <valid::u16::Data as TryInto<u16>>::try_into(head).unwrap() - 1;
        let crossing = ArtinGenerator::try_new(crossing_foot, sign).unwrap();
        let left: Vec<ArtinGenerator> = (band_foot..crossing_foot)
            .into_iter()
            .map(|foot_idx| ArtinGenerator::try_new(foot_idx, Sign::Negative).unwrap())
            .collect();
        let right: Vec<ArtinGenerator> = left.clone().iter().rev().map(|a| a.inverse()).collect();
        (band_generator, [left, vec![crossing], right].concat())
    })
}

fn of_band_letter_with_canonical_decomposition(
    max_head: Option<u16>,
    max_height: Option<u16>,
    max_artin_length: Option<u16>,
) -> impl Strategy<Value = (Letter, Vec<Letter>)> {
    ...
    of_band_generator_with_canonical_decomposition(max_head, max_height, max_artin_length)
        .prop_perturb(|(band, artin_generators), mut rng| {
            let band_letter = Letter::Band(band.unwrap());
            let artin_letters: Vec<Letter> = artin_generators
                .iter()
                .map(|&a| {
...
fn of_band_letter_walled_with_canonical_decomposition(
    max_head: Option<u16>,
    max_height: Option<u16>,
    max_artin_length: Option<u16>,
) -> impl Strategy<Value = WalledBandLetterCoalescence> {
    ...
    of_band_letter_with_canonical_decomposition(max_head, max_height, Some(max_band_artin_length))
        .prop_flat_map(move |(band, decomposed_band)| {
            let max_wall_length = max_artin_length.unwrap_or(u16::MAX)
                - <usize as TryInto<u16>>::try_into(decomposed_band.len()).unwrap();
            if max_wall_length < 2 {
...
fn of_band_letter_walled_multiple_with_canonical_decomposition(
    max_head: Option<u16>,
    max_height: Option<u16>,
    max_artin_length: Option<u16>,
) -> impl Strategy<Value = Vec<WalledBandLetterCoalescence>> {
    ...
    (1..=max_walled_coalescences).prop_flat_map(move |num_walled_coalescences| {
        ...
        let mut strategies = Vec::new();
        for _ in 0..num_walled_coalescences {
            strategies.push(of_band_letter_walled_with_canonical_decomposition(
                max_head,
                max_height,
                Some(max_walled_coalescence_artin_length),
            ));
...
pub fn of_word_with_canonical_decomposition(
    max_head: Option<u16>,
    max_height: Option<u16>,
    max_artin_length: Option<u16>,
    prune: bool,
    ...
    of_band_letter_walled_multiple_with_canonical_decomposition(
        max_head,
        max_height,
        max_artin_length,
    )
    .prop_perturb(move |mut walled_coalescences, mut rng| {
        let mut walled_coalescences = walled_coalescences.iter_mut().peekable();
        let mut coalescence: Vec<Letter> = Vec::new();
        let mut decomposed: Vec<Letter> = Vec::new();
        let mut first_left_letter: Option<Letter> = None;
...
pub mod test_cases {
    ...
    pub fn decompose_band(
        ...
        of_band_generator_with_canonical_decomposition(max_head, max_height, max_artin_length)
            .prop_map(|(band_generator, artin_generators)| DecomposeBand {
    ...
    pub fn decompose_letter(
        ...
        prop_oneof![
            of_band_generator_with_canonical_decomposition(max_head, max_height, max_artin_length)
                .prop_map(|(band_generator, artin_generators)| DecomposeLetter {
    ...
    pub fn decompose_word(
        ...
        of_word_with_canonical_decomposition(max_head, max_height, max_artin_length, prune)
            .prop_map(|(expected, word)| DecomposeWord {
    ...
    pub fn decompose_braid(
        ...
        of_word_with_canonical_decomposition(max_braid_index, max_height, max_artin_length, prune)
            .prop_map(|(expected_word, coalesced_word)| {
